...
const initializeFormPage = (recipeId) => {
    ...
    //create each ingredient list item with an id for each ingredient line, so we can delete it or check it off 
...
const renderRecipes = () => {
    ...
    const recipes = getRecipes();
    const filteredRecipes = recipes.filter(recipe => recipe.title.toLowerCase().includes(filters.searchText.toLocaleLowerCase()));
